  ...
  const verifyOtp = async () => {
    ...
    const jwtLogin = async () => {
      ...
      try {
        ...
        } else if (key && iid) navigate(`/join/${iid}/${key}`);
  ...
  const handleKeyDownCP = (e) => {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
          teacherLogin.classList.remove("opacity-0");
          teacherLogin.classList.remove("scale-50");
          teacherLogin.classList.add("fadeinround");
        }, 100)

        return () => {
          teacherLogin.classList.remove("fadeinround")
          clearTimeout(timer);
        }
    }, [])

    return (
        <div id="teacherLogin" className="opacity-0 scale-50 teacherLogin bg-cover bg-center flex flex-col items-center justify-center bg-no-repeat w-screen h-screen overflow-hidden">
           {/* Forget Password code added */}
      {
        forgetPassWindow ?
          ...
          :
          <div className={`duration-500 animate__animated animate__fadeInUpBig animate__slow h-screen w-screen flex flex-col items-center justify-center ${
            panel === "register" || i18n.language !== "en" ? '' : 'mt-[140px]'
          }`}>
            ...
            <UnauthenticatedTemplate>
              {i18n.language === "en" ? (
                <div style={{ bottom: panel === 'register' ? '' : 'calc(50% + 200px)' }} className={`duration-500 text-center absolute ${panel === 'register' ? 'top-[-10px] left-[-80px] scale-50' : 'left-1/2 transform -translate-x-1/2'}`}>
                    ...
                    <p className="font-bold text-white mb-1 leading-none text-[100px]">Teachers</p>
                </div>
                  ) : (
                      ...
                      <>
                        ...
                        <div className="relative w-11/12 md:w-[500px]">
                          ...
                          {/* <div className="flex justify-end">
                            <button className="mt-2 text-white underline cursor-pointer text-center" disabled={sender} onClick={handleForgotPassword}>Forgot Password?</button>
                          </div> */}
                        </div>
